...
#include "gromacs/random/tabulatednormaldistribution.h"  //Shivani
#include "gromacs/random/threefry.h"  //Shivani
...
//    std::default_random_engine generator;
    ...
    double x1=distribution(gen) ;
    double x2=distribution(gen) ;
    //double s ;
    x2=1.0-x2 ;
    z1=1.0*sqrt(-2.0*log(x2))*cos(2.0*3.14*x1) ;
    printf("%f\n",z1);
...
void gmx::Integrator::do_md()
{
    ...
    double z1;// Shivani
    for (int n_loop = 0; n_loop < 5000; n_loop++)
    {
     gauss_dist(z1);     
    }   //n_loop
...
/*   ------------------------------------- Edited by Shivani   ---------------------------------*/
     ...
     real friction = 10.0, k_bol = 8.314e+21;
     real temp_lambda =300.0; 
     real k_bol_t = top_global->natoms* k_bol * temp_lambda;
     real c1 = exp(-0.5*friction*time_step);
     real c2 = sqrt(k_bol_t*(1.0-c1*c1)/m_lambda);
...
//        gauss_dist(z1);
//        running_v_lambda=c1*running_v_lambda+c2*z1;
//        gauss_dist(z1);
//        running_v_lambda=c1*running_v_lambda+c2*z1;
